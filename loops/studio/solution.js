...
function mealAssembly(protein, grains, veggies, beverages, desserts, numMeals) {
  ...
  // "meals" = total meals created. using meal and meals as variable names is a bad idea
  let totalMealsCreated = [];
...
for (let i = 0; i < numMeals; i++) {
  ...
  let meal = [];
    ...
    totalMealsCreated.push(meal);
}
  return totalMealsCreated;
}
...
function askForNumber() {
  ...
  while (numMeals < 1 || numMeals > 6 || isNaN(numMeals) ) {
      numMeals = input.question("Number 1-6 only ")
  }
  return numMeals;
}
